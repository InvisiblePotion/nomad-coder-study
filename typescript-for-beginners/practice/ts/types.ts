...
/* [Type] */
// name은 readonly, age는 optional이다
type NewType = {
...
const typeMaker = (name: string): NewType => ({ name });
...
const noName: NewType = { name: " ", age: 25 };

/* [Union] */
type NewUnion = number | string;
const uni1: NewUnion = 1;
const uni2: NewUnion = "2";

// 직접 명시하는 경우도 문제 없다
const uni3: number | string = 1;

// primitive 타입이 아니라도 가능하다
const uni4: NewType | boolean = { name: "uni4", age: 4 };

/* [Intersection] */
type A_type = { a: number };
type B_type = { b: string };
type NewInter = A_type & B_type;
const inter: NewInter = { a: 1, b: "2" };

// 이런식으로 익명 객체를 이용한 타입 조합도 가능하다
type NewInter2 = NewInter & { c: boolean };
const inter2: NewInter2 = { a: 1, b: "2", c: true };

/* [Tuple] */
type NewTuple = [number, string, boolean];
const tup: NewTuple = [1, "2", true];

/* [unknown] */
let apiResult: unknown;
if (typeof apiResult === "number") {
    const resultNumber: number = apiResult + 1;
}

/* [never] */
function whyNever(whyNot: number | string) {
...
/* [void] */
const itsVoid: (a: string) => void = (a) => console.log(a);